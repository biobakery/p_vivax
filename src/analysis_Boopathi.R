...
df.exprs <- exprs(eset.Boopathi)
df.pdata <- pData(eset.Boopathi)
df.pdata$source <- gsub('P. vivax parasites from patient\'s blood_', 
                        '',
                        df.pdata$source_name_ch1,
                        fixed = T)

# platform annotation
df.probes <- getGEO(filename = 'data/GPL18382.soft') %>% Table
  ...
  c('rings', 'IG', 'MG', 'commit') %>% 
    sapply(function(i.stage) {
      df.clst %>% filter(stage == i.stage) %>% '['(, 'cluster') %>% 
        '['(l.clst.pv, .) %>% unlist %>% '%in%'(df.probes$ORF, .) %>% 
        '['(df.probes$ID, .) %>% 
        '['(df.exprs, ., ) %>% 
        apply(2, mean) %>% return
    }) %>% data.frame,
...
df.exprs.avg.long <- gather(cbind(df.exprs.avg, df.pdata), 
                            key = stage,
...
pdf('result/Boopathi/boxplot_stage_avg.pdf', width=8, height=6)
ggplot(df.exprs.avg.long,
       aes(x=stage, y=mean_expression, color = source)) +
